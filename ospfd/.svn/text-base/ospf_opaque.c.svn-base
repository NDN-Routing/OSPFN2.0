...
struct ospf_opaque_functab
{
  ...
  /* 08/23/2011 yic +1 */
  void (* router_routing_table_change_hook)(struct in_addr router_id, struct route_node *rn);
};
...
ospf_register_opaque_functab (
  ...
  int (* del_lsa_hook)(struct ospf_lsa *lsa),
  /* 08/23/2011 yic +1 */
  void (* router_routing_table_change_hook)())
{
  struct list *funclist;
  ...
  /* 08/23/2011 yic +1 */
  new->router_routing_table_change_hook = router_routing_table_change_hook;
...
/* 08/23/2011 yic +n */
static void opaque_router_routing_table_change_callback(struct list *funclist,
        struct in_addr router_id, struct route_node *rn)
{
    ...
    /* This function handles ALL types of LSAs, not only opaque ones. */
    for (ALL_LIST_ELEMENTS (funclist, node, nnode, functab))
        if (functab->router_routing_table_change_hook != NULL)
            (* functab->router_routing_table_change_hook)(router_id, rn);
}
...
/* 08/23/2011 yic +n */
void ospf_opaque_router_routing_table_change(struct in_addr router_id, struct route_node *rn)
{
    struct list *funclist;

    funclist = ospf_opaque_wildcard_funclist;
    opaque_router_routing_table_change_callback(funclist, router_id, rn);

...
ospf_apiserver_init (void)
{
  ...
  rc =
    ospf_register_opaque_functab (0 /* all LSAs */, 
                  ...
                  /* 08/23/2011 yic +1 */
                  ospf_apiserver_notify_router_routing_table_change);
  if (rc != 0)
    {
...
ospf_apiserver_send_msg (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_NSM_CHANGE:
    /* 08/25/2011 yic+ */
    case MSG_ROUTER_ROUTING_TABLE_CHANGE:
...
ospf_apiserver_handle_msg (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_DELETE_REQUEST:
      ...
      break;
    case MSG_GET_ROUTER_NEXTHOPS_REQUEST:
      rc = ospf_apiserver_handle_get_router_nexthops_request(apiserv, msg);
      break;
    default:
				     ...
				     u_char lsa_type, u_char opaque_type)
{
  ...
  rc =
    ospf_register_opaque_functab (lsa_type, opaque_type,
                  ...
                  /* 08/23/2011 yic +1 */
                  NULL /* ospf_apiserver_router_routing_table_change */);
...
/* 08/25/2011 yic+ */
static int apiserver_get_router_nexthops_callback(struct ospf_apiserver *apiserv,
        struct in_addr router_id, int seqnum)
{
    struct msg *msg;
    struct ospf *ospf;
    int rc = -1;
    ...
    ospf = ospf_lookup();
    rn = route_node_match_ipv4(ospf->new_rtrs, &router_id);

    msg = new_msg_router_routing_table_change(seqnum, router_id, rn);

    if (!msg)
    {
        zlog_warn ("apiserver_get_router_nexthops_callback failed");
        goto out;
    }

    rc = ospf_apiserver_send_msg(apiserv, msg);
    msg_free(msg);
...
/* 08/25/2011 yic+ */
int ospf_apiserver_handle_get_router_nexthops_request(struct ospf_apiserver *apiserv,
        struct msg *msg)
{
    int rc = 0;
    u_int32_t seqnum;
    struct msg_get_router_nexthops_request *smsg;

    seqnum = msg_get_seq (msg);
    smsg = (struct msg_get_router_nexthops_request *)STREAM_DATA(msg->s);

    rc = apiserver_get_router_nexthops_callback(apiserv,
            smsg->router_id, seqnum);
...
/* 08/23/2011 yic +n */
void ospf_apiserver_notify_router_routing_table_change(struct in_addr router_id, struct route_node *rn)
{
    struct msg *msg;

    msg = new_msg_router_routing_table_change(0, router_id, rn);
    if (msg != NULL)
    {
        ospf_apiserver_clients_notify_all(msg);
        msg_free(msg);

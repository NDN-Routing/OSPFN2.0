...
ospf_api_typename (int msgtype)
{
  struct nametab NameTab[] = {
    ...
    /* 08/25/2011 yic+ */
    { MSG_GET_ROUTER_NEXTHOPS_REQUEST, "Get router nexthop",},
    { MSG_REPLY,                 "Reply",                  },
    ...
    /* 08/25/2011 yic+ */
    { MSG_ROUTER_ROUTING_TABLE_CHANGE, "Router routing table change",},
  };
...
/* 08/25/2011 yic+ */
struct msg *new_msg_get_router_nexthops_request(u_int32_t seqnum, struct in_addr router_id)
{
    struct msg_get_router_nexthops_request rmsg;
    rmsg.router_id = router_id;
    return msg_new(MSG_GET_ROUTER_NEXTHOPS_REQUEST, &rmsg, seqnum, sizeof(struct msg_get_router_nexthops_request));
}
...
/* 08/24/2011 yic+ */
struct msg *new_msg_router_routing_table_change(u_int32_t seqnr,
        struct in_addr router_id, struct route_node *rn)
{
    struct msg_router_routing_table_change *nmsg, *tmsg;
    struct msg *result;
    ...
    size = sizeof(struct msg_router_routing_table_change) - sizeof(struct in_addr);
    nmsg = malloc(size);
    nmsg->router_id = router_id;
    nmsg->nexthop_count = 0;
    ...
    if (rn && rn->info)
    {
        for (ALL_LIST_ELEMENTS_RO((struct list *)rn->info, node, or))
        {
            for (ALL_LIST_ELEMENTS_RO(or->paths, pnode, path))
            {
                ...
                tmsg = realloc(nmsg, size);
                assert(tmsg);
                nmsg = tmsg;
                memcpy(&nmsg->nexthops[nmsg->nexthop_count], &path->nexthop, sizeof(struct in_addr));
                nmsg->nexthop_count ++;
            }
        }
    }

    nmsg->nexthop_count = htonl(nmsg->nexthop_count);

    result = msg_new(MSG_ROUTER_ROUTING_TABLE_CHANGE, nmsg, seqnr, size);
    free(nmsg);
    return result;
}

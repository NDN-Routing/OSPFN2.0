...
/* 08/25/2011 yic+ */
int ospf_apiclient_get_router_nexthops(struct ospf_apiclient *oclient,
        struct in_addr router_id)
{
    struct msg *msg;
    int rc;

    msg = new_msg_get_router_nexthops_request(ospf_apiclient_get_seqnr(),
            router_id);
...
/* 08/25/2011 yic+ */
static void ospf_apiclient_handle_router_routing_table_change(struct ospf_apiclient *oclient, struct msg *msg)
{
    struct msg_router_routing_table_change *rc;
    struct in_addr router_id;
    int nexthop_count;
    struct in_addr *nexthops;

    if (oclient->router_routing_table_change_notify)
    {
        rc = (struct msg_router_routing_table_change *)STREAM_DATA(msg->s);

        router_id = rc->router_id;
        nexthop_count = ntohl(rc->nexthop_count);

        if (nexthop_count > 0)
        {
            nexthops = XMALLOC(MTYPE_OSPF_APICLIENT, nexthop_count * sizeof(struct in_addr));
            memcpy(nexthops, &rc->nexthops[0], nexthop_count * sizeof(struct in_addr));
        }
        else
            nexthops = NULL;

        (oclient->router_routing_table_change_notify)(router_id, nexthop_count, nexthops);

        if (nexthop_count > 0)
            XFREE(MTYPE_OSPF_APICLIENT, nexthops);
    }
}
...
ospf_apiclient_msghandle (struct ospf_apiclient *oclient, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_ROUTER_ROUTING_TABLE_CHANGE:
      ospf_apiclient_handle_router_routing_table_change(oclient, msg);
      break;
    default:
...
ospf_apiclient_register_callback (struct ospf_apiclient *oclient,
				  ...
				  void (*delete_notify) (struct in_addr
							 ...
							 lsa),
                  /* 08/25/2011 yic+ */
				  void (*router_routing_table_change_notify)(
                          struct in_addr router_id,
                          int nexthop_count,
                          struct in_addr *nexthops))
{
  ...
  /* 08/25/2011 yic+ */
  oclient->router_routing_table_change_notify = router_routing_table_change_notify;
}
